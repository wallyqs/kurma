...
func TestHostNetworkNamespaceIsolator(t *testing.T) {
	...
	manager.networkManager = &mocks.NetworkManager{
		ProvisionFunc: func(_ backend.Pod, _ []string) (string, []*ntypes.IPResult, error) {
...
func TestHostPriviledgeIsolator(t *testing.T) {
	...
	// Validate state
	tt.TestEqual(t, len(pod.options.RawVolumes), 3)
	tt.TestEqual(t, len(pod.options.StagerMounts), 3)
...
func TestHostApiIsolator(t *testing.T) {
	tt.StartTest(t)
	defer tt.FinishTest(t)
	...
	isolatorJson := `[{"name":%q,"value":true}]`
	err := json.Unmarshal([]byte(fmt.Sprintf(isolatorJson, kschema.HostApiAccessName)), &pod.manifest.Pod.Apps[0].App.Isolators)
	tt.TestExpectSuccess(t, err)
	tt.TestEqual(t, len(pod.manifest.Pod.Apps[0].App.Isolators), 1)
	tt.TestEqual(t, pod.manifest.Pod.Apps[0].App.Isolators[0].Name.String(), kschema.HostApiAccessName)
	...
	// Validate state
	tt.TestEqual(t, len(pod.options.RawVolumes), 1)
	tt.TestEqual(t, len(pod.options.StagerMounts), 1)
	tt.TestEqual(t, len(pod.manifest.Pod.Apps[0].Mounts), 1)
	tt.TestEqual(t, len(pod.manifest.Pod.Volumes), 1)

	// Validate the container configuration
	config := pod.stagerContainer.Config()
	tt.TestNotEqual(t, config, nil)
	tt.TestEqual(t, len(config.Mounts) > 1, true, "Should have at least 1 mount")

	_, err = os.Stat(filepath.Join(pod.stagerRootPath(), "volumes", "example-kurma-socket"))
	tt.TestExpectSuccess(t, err)

	mount := config.Mounts[len(config.Mounts)-1]
	tt.TestEqual(t, mount.Destination, "/volumes/example-kurma-socket/kurma.sock")

	tt.TestEqual(t, pod.manifest.Pod.Apps[0].Mounts[0].Volume.String(), "example-kurma-socket")
	tt.TestEqual(t, pod.manifest.Pod.Apps[0].Mounts[0].Path, "/var/lib/kurma")
}
